package com.gpaglia.scalatest.support;

import com.gpaglia.scalatest.framework.ScalatestTestClassScanner;
import com.gpaglia.scalatest.framework.ScalatestTestFramework;
import com.gpaglia.scalatest.framework.processors.ScalatestPatternMatchTestClassProcessor;
...
public class ScalatestTestExecuter implements TestExecuter<ScalatestTestExecutionSpec> {
  private static final Logger LOGGER = Logging.getLogger(ScalatestTestExecuter.class);
  ...
  @Override
  public void execute(final ScalatestTestExecutionSpec testExecutionSpec, TestResultProcessor testResultProcessor) {
    final ScalatestTestFramework testFramework = testExecutionSpec.getTestFramework();
    ...
    processor =
        new ScalatestPatternMatchTestClassProcessor(testFilter,
    ...
    if (! testExecutionSpec.isScanForTestClasses()) {
      LOGGER.warn("Found a disabled scanForTestClasses -- forced to true!!");
    }
    ...
    detector = new ScalatestTestClassScanner(testClassFiles, testFrameworkDetector, processor);
  ...
  private int getMaxParallelForks(ScalatestTestExecutionSpec testExecutionSpec) {
